                    ...
                    <tbody className="text-slate-700">
                        {sortedRoomEntries.map(([roomName, items]) => (
                            <React.Fragment key={roomName}>
                                ...
                                {/* Items Rows */}
                                {items.map((row: any) => (
                                    <tr key={row.id} className={`hover:bg-sky-50/30 transition-colors ${row.hasDiff ? 'bg-amber-50/40' : ''}`}>
                                        <td className="p-2 pl-4 border-b border-r border-slate-200 font-medium truncate sticky left-0 bg-white group-hover:bg-sky-50/30 z-10">
                                            {row.name}
