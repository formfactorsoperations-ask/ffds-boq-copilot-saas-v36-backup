    ...
    return (
        <div className="flex flex-col min-h-[calc(100vh-6rem)] max-w-[1600px] mx-auto font-sans relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 pt-4">
                ...
                <div className="flex flex-wrap items-center gap-4 md:bg-white md:p-3 md:rounded-2xl md:border md:border-slate-100 md:shadow-sm">
                    {/* Auto Stage Completion Checkbox Toggle */}
                    <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-100/80">
                        <input 
                            type="checkbox" 
                            ...
                            className="w-4 h-4 text-[#3D52A0] border-slate-300 rounded focus:ring-[#3D52A0] cursor-pointer accent-[#3D52A0]"
                        />
                        <label htmlFor="autoStageCompletionToggle" className="text-[10px] font-bold uppercase tracking-widest text-slate-600 cursor-pointer select-none">
                    ...
                    <div className="flex flex-col items-start md:items-end md:mr-2">
                        ...
                        <div className="flex items-center gap-4 w-48 md:w-64">
                            <div className="flex-1 h-2 rounded-full bg-slate-100 overflow-hidden">
                                <div className="h-full bg-[#3D52A0] transition-all duration-1000" style={{ width: `${overall.pct}%` }} />
                            </div>
                        ...
                        {PHASES.map((p, i) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {isActive && <div className="hidden md:block absolute inset-y-1 left-2 right-2 bg-sky-50 rounded-xl -z-10" />}
                                    ...
                                    <div className="hidden md:flex shrink-0 w-8 h-8 rounded-full items-center justify-center bg-white border-2 z-10 transition-colors duration-300" 
                                         style={{ borderColor: isActive ? '#3D52A0' : isPast ? '#10b981' : '#e2e8f0' }}>
                                        {isPast && !isActive ? (
                                            <Check className="w-4 h-4 text-emerald-500" />
                                        ) : (
                                            <div className={`w-2.5 h-2.5 rounded-full transition-colors ${isActive ? 'bg-[#3D52A0]' : isPast ? 'bg-emerald-500' : 'bg-slate-200'}`} />
                                        )}
                                    </div>

                                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                                        <div className="flex items-center justify-between gap-2 mb-1">
                                            <span className={`text-[9px] font-bold uppercase tracking-widest ${isActive ? 'text-[#3D52A0]' : 'text-slate-400'}`}>
                                                Phase 0{i + 1}
                                            </span>
                                            {isCurrentEngine && <span className="text-[8px] font-bold uppercase tracking-widest text-[#334486] bg-sky-50 px-1.5 py-[2px] rounded border border-sky-200">Active</span>}
                                        </div>
                                        <div className={`text-[13px] tracking-tight truncate ${isActive ? 'font-semibold text-slate-900' : 'font-medium text-slate-500'}`}>
                                            {p.name}
                                        </div>
                                    </div>
                                </button>
                    ...
                    <div className="max-w-4xl flex flex-col h-full pl-0 md:pl-2">
                        ...
                        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                            ...
                            {(pendingSteps.length > 0 || lockedSteps.length > 0) && (
                                ...
                                >
                                    <div className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${retrofitMode ? 'bg-red-50 text-red-600' : 'bg-slate-200/50 text-slate-500 group-hover:bg-[#334486] group-hover:text-white'}`}>
            ...
            <div className="flex items-start gap-4 md:gap-5 flex-1 min-w-0 md:pr-6">
                <div className="mt-0.5 shrink-0 flex items-center justify-center">
                    {isDone ? (
                        ...
                        <div className="w-6 h-6 rounded-full border-2 border-[#3D52A0] bg-[#3D52A0] flex items-center justify-center shadow-inner">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 mb-1.5">
                        ...
                        {step.isAutoDerived && (
                            <span className="text-[8px] font-bold uppercase tracking-widest text-[#334486] bg-sky-50 px-1.5 py-0.5 rounded border border-sky-100 w-max">Auto Validated</span>
                        )}
                    </div>
            ...
            <div className="flex items-center gap-3 shrink-0 pt-5 md:pt-0 pl-9 md:pl-0 border-t border-slate-100 md:border-t-0 mt-4 md:mt-0">
                {step.linkedFeature && (
                    <button 
                        ...
                        className={`text-[10px] font-bold uppercase tracking-widest px-4 py-2.5 rounded-xl transition-all flex items-center gap-2 ${step.linkedTab && !locked ? 'text-[#334486] bg-sky-50 hover:bg-sky-100 shadow-sm border border-sky-100' : 'text-slate-400 bg-transparent border border-transparent cursor-not-allowed'}`}
                    >
                ...
                {!isDone && onInteract && (
                    <button 
                        onClick={onInteract}
                        className="text-[10px] font-bold uppercase tracking-widest px-5 py-2.5 rounded-xl bg-[#3D52A0] text-white hover:bg-[#334486] transition-all shadow-sm hover:shadow"
                    >
                        Sign Off
                    </button>
                )}
