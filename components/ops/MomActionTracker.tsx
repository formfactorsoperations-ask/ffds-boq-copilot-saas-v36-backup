...
export function MomActionTracker({
  ...
  const deleteMom = async (momId: string) => {
    if (
      window.confirm(
        "Are you sure you want to delete this Minutes of Meeting? This action cannot be undone.",
      )
    ) {
      try {
        await deleteDoc(
  ...
  const openCount = moms.reduce(
    (acc, m) =>
      acc + (m.actionItems?.filter((a) => a.status === "open").length || 0),
    0,
  );

  if (loading) {
    return (
      <div className="p-8 text-center text-slate-500">Loading Tracker...</div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative min-h-[500px] flex flex-col">
      <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-indigo-950 font-serif">
            MOMs & Actions
          </h2>
          {viewMode === "actions" ? (
            <p className="text-sm text-slate-500">
              {openCount} open actions across all meetings
            </p>
          ) : (
            <p className="text-sm text-slate-500">
              {moms.length} total meetings recorded
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button
              ...
              className={`px-4 py-1.5 text-xs font-bold uppercase tracking-wider rounded-md transition ${viewMode === "actions" ? "bg-white shadow text-indigo-950" : "text-slate-500 hover:text-slate-700"}`}
            >
              Action Tracker
            </button>
            <button
              onClick={() => setViewMode("history")}
              className={`px-4 py-1.5 text-xs font-bold uppercase tracking-wider rounded-md transition ${viewMode === "history" ? "bg-white shadow text-indigo-950" : "text-slate-500 hover:text-slate-700"}`}
            >
              MOM History
            </button>
          </div>
        ...
        {viewMode === "actions" ? (
                ...
                {filtered.map(({ action, mom }, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      className="hover:bg-indigo-50/30 transition-colors border-b border-slate-50"
                    >
                      <td className="py-3 px-4">
                        <button
                          ...
                          className="flex items-center justify-center p-1 hover:scale-110 transition cursor-pointer"
                        >
                          {action.status === "done" ? (
                            <CheckCircle2
                              size={20}
                      ...
                      <td className="py-3 px-4 max-w-sm">
                        <span
                          className={`font-medium ${action.status === "done" ? "line-through text-slate-400" : "text-indigo-900"}`}
                        >
                          {action.text}
                        </span>
                        {action.flags?.scope && (
                          <span className="ml-2 inline-flex items-center px-1.5 py-0.5 bg-red-100 text-red-700 text-[10px] rounded-md uppercase font-bold tracking-widest leading-none">
                            Scope
                          </span>
                        )}
                      </td>
                      <td className="py-3 px-4 align-middle">
                        <span
                          className={`inline-flex items-center justify-center px-2 pt-1 pb-[3px] rounded-md text-[10px] font-bold uppercase tracking-wider leading-none ${action.owner === "client" ? "bg-purple-100 text-purple-700" : "bg-slate-200 text-slate-700"}`}
                        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
            {moms.map((mom) => (
              <div
                key={mom.id}
                className="border border-slate-200 rounded-xl p-5 hover:shadow-md transition-shadow bg-white flex flex-col justify-between group"
              >
                <div>
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h3 className="font-bold text-indigo-950 group-hover:text-indigo-600 transition-colors">
                        {mom.momRef}
                      </h3>
                  ...
                  <div className="flex gap-4 text-xs text-slate-600 mb-4 bg-slate-50 p-3 rounded-lg border border-slate-100">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-semibold text-amber-600">
                        {mom.actionItems?.filter((a) => a.status === "open")
                          .length || 0}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between mt-2 pt-3 border-t border-slate-100">
                  <button
                    onClick={() =>
                      onOpenMom ? onOpenMom(mom.id) : setSelectedMomId(mom.id)
                    }
...
function FilterButton({ active, onClick, children }: any) {
  return (
    <button
      onClick={onClick}
      className={`px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-md transition ${active ? "bg-white shadow text-indigo-950" : "text-slate-500 hover:text-slate-700"}`}
    >
      {children}
    </button>
  );
