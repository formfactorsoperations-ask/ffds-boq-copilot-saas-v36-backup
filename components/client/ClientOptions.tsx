...
const ClientOptions: React.FC<ClientOptionsProps> = ({ tiers, comparisonData, content, activeTierId, onSelectTier }) => {
    ...
    const data = content || {
        title: tiers.length > 1 ? "Options Overview" : "Proposed Configuration",
        subtitle: tiers.length > 1 
            ? "Each option changes finish level, detailing, and civil scope. Core functionality remains intact."
            : "A detailed overview of the proposed finish level, detailing, and civil scope for your project."
    };
    ...
    return (
        <section id="options" className="rounded-3xl border border-slate-200 bg-white shadow-sm p-6 md:p-8">
            ...
            <div className={`mt-6 grid grid-cols-1 gap-5 ${tiers.length > 1 ? 'lg:grid-cols-3' : 'max-w-md mx-auto'}`}>
                {tiers.map(tier => {
                    ...
                    return (
                        <div 
                            ...
                            className={`tier-option-card rounded-3xl p-6 flex flex-col cursor-pointer transition-all duration-300 relative
                                ${isActive 
                                    ? 'border-2 border-sky-900 bg-white shadow-lg scale-[1.02] z-10 ring-2 ring-slate-100' 
                                    : isRecommended 
                        ...
                        >
                            ...
                            <div className={`viewing-badge absolute -top-3 left-1/2 -translate-x-1/2 bg-sky-900 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest shadow-md ${isActive ? '' : 'hidden'}`}>
                                Viewing Details
                            </div>

                            <div className="flex items-center justify-between">
                                <div>
                                    <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{tiers.length > 1 ? (tier.name === "Essential Elegance" ? "Option A" : tier.name === "Comfort Upgrade" ? "Option B" : "Option C") : "Standard Spec"}</div>
