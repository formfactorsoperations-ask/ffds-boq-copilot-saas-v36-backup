...
const ClientComparisonMatrix: React.FC<ClientComparisonMatrixProps> = ({ data, tierNames, isLoading }) => {
  ...
  return (
    <div className="border-2 border-[#334486] rounded-xl overflow-hidden shadow-sm">
        {/* Header Row - Updated to Slate-900 for uniform black theme */}
        <div className="grid grid-cols-[180px_1fr] md:grid-cols-[220px_1fr_1fr_1fr] bg-[#3D52A0]/90 backdrop-blur-md border border-white/20 text-white">
            <div className="p-4 font-bold text-xs uppercase tracking-widest flex items-center">Specification</div>
            {tierNames.map((name, idx) => (
                <div key={idx} className="p-4 font-bold text-xs uppercase tracking-widest border-l border-slate-700 flex items-center justify-center text-center bg-[#3D52A0]/90 backdrop-blur-md border border-white/20">
                    {name}
                </div>
            ))}
        </div>
