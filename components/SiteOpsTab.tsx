...
interface SiteOpsTabProps {
    ...
    onAddCalculatedItem?: (name: string, category: string, qty: number, unit: string, roomId: string) => void;
    initialModule?: 'execution' | 'decision-tracker' | 'action-tracker' | 'client-updates' | 'design-docs' | 'tiling' | 'electrical' | 'carpentry' | 'checklist' | 'vault';
}
...
const SiteOpsTab: React.FC<SiteOpsTabProps> = ({ 
    ...
    onProjectUpdate, 
    onAddCalculatedItem, 
    initialModule = 'execution' 
}) => {
    const { orgData } = useOrg();

    // Map legacy / initial tabs gracefully into our core 5 pillars
    const [activeModule, setActiveModule] = useState<'execution' | 'client-updates' | 'action-tracker' | 'decision-tracker' | 'vault'>(() => {
        if (initialModule === 'design-docs' || initialModule === 'checklist' || initialModule === 'vault') {
            return 'vault';
        }
        if (initialModule === 'tiling' || initialModule === 'electrical' || initialModule === 'carpentry') {
            return 'execution';
        }
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 print:hidden">
                <div>
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                        <span className="p-2 bg-slate-900 text-white rounded-2xl shadow-sm">
                            <LayoutDashboard className="w-6 h-6" />
                        </span>
                        Execution Control Room
                    </h2>
                    <p className="text-slate-500 font-medium text-sm mt-1 ml-1">
                        Form Factors execution intelligence layer. Driving decisions, logs, and site alignment.
                    </p>
                </div>
            </div>
