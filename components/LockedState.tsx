...
export default function LockedState({
    ...
    return (
        <div className="flex flex-col items-center justify-center p-8 md:p-12 text-center h-full min-h-[50vh] bg-slate-50/50 rounded-2xl border border-slate-100">
            <div className={`w-16 h-16 rounded-full mb-6 flex items-center justify-center shadow-sm ${isEmptyState ? 'bg-sky-100 text-[#0066CC]' : 'bg-slate-100 text-slate-400'}`}>
            ...
            {onAction && (
                <button
                    onClick={onAction}
                    className={`flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold shadow-sm transition-all ${isEmptyState ? 'bg-[#0066CC] text-white hover:bg-[#0055B3]' : 'bg-white border border-slate-200 text-slate-700 hover:border-slate-300 hover:bg-slate-50'}`}
                >
