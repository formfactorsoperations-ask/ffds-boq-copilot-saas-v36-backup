      ...
      <div className="flex items-center gap-2 flex-wrap">
          ...
          {(['day', 'week', 'month'] as Zoom[]).map(z => (
            <button key={z} onClick={() => setZoom(z)}
              className={`px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wider transition-colors ${
                zoom === z ? 'bg-[#0066CC]/90 backdrop-blur-md border border-white/20 text-white' : 'text-slate-500 hover:text-slate-800'}`}>
              {z}
            </button>
          ))}
        </div>
        <button onClick={() => setShowBaseline(s => !s)}
          className={`px-3 py-2 rounded-xl text-[11px] font-bold border transition-colors ${
            showBaseline ? 'border-sky-200 text-[#0055B3] bg-sky-50' : 'border-slate-200 text-slate-500 bg-white'}`}>
          ▤ Baseline
        </button>
          ...
          <>
            ...
            {!schedule.baselineAt && (
              <button onClick={() => onChange(freezeBaseline(schedule))}
                className="px-3 py-2 rounded-xl text-[11px] font-bold border border-sky-200 text-[#0055B3] bg-white">
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${
                            selected.status === status
                              ? 'bg-[#0066CC]/90 backdrop-blur-md border border-white/20 text-white border-[#0055B3]'
                  ...
                  <div className="flex gap-1.5 mt-2 flex-wrap">
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-bold border ${
                        selected.pinned ? 'bg-[#0066CC]/90 backdrop-blur-md border border-white/20 text-white border-[#0055B3]' : 'bg-white text-slate-600 border-slate-200'}`}
                    >
                        ...
                        {(['design', 'execution', 'procurement'] as const).map(k => (
                          <button
                            ...
                            className={`py-1 px-2 rounded-lg text-[10px] font-bold uppercase transition-colors border ${
                              selected.kind === k
                                ? 'bg-sky-50 text-[#0055B3] border-sky-300 shadow-xs'
      ...
      {markerDay && byDay.get(markerDay) && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setMarkerDay(null)} className="text-[11px] font-bold text-slate-400 hover:text-[#0055B3]">
              Close
            </button>
          </div>
...
function HoldDialog({ scope, trades, onCancel, onApply }: {
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-[#0066CC]/90 backdrop-blur-md border border-white/20/40 backdrop-blur-sm flex items-center justify-center p-5"
      ...
      <div className="bg-white rounded-2xl w-full max-w-md p-5 shadow-2xl">
        ...
        <div className="flex gap-2 mt-4">
          <button
            ...
            className="flex-1 px-4 py-2.5 rounded-xl bg-[#0066CC]/90 backdrop-blur-md border border-white/20 text-white text-sm font-bold">
            Apply hold
          </button>
  ...
  return (
    <div className="bg-white border border-[#EBEAE5] rounded-[24px] p-6 shadow-xs text-left mt-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6 border-b border-slate-100 pb-4">
        ...
        <div className="px-3 py-1 bg-sky-50/80 text-[#0066CC] text-[10px] uppercase tracking-wider font-extrabold rounded-full border border-sky-100 flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-slate-50/70 rounded-2xl border border-slate-200/60 p-4 space-y-3">
          ...
          <div className="p-3 bg-white border border-slate-200/80 rounded-xl shadow-xs space-y-2.5">
            ...
            <div className="h-5 w-full bg-slate-100 rounded-lg flex overflow-hidden relative border border-slate-200/80 p-0.5">
              <div className="h-full bg-[#0066CC] text-white text-[9px] font-bold rounded-md flex items-center justify-center px-2 transition-all" style={{ width: '68%' }}>
                Active Execution
              </div>
              {overrun != null ? (
                overrun > 0 ? (
          ...
          <p className="text-[11px] text-slate-500 leading-normal">
            {isCompressed ? (
              <span className="text-slate-800 font-semibold">
                ⚡ <strong className="text-[#0066CC]">Auto-Pacing Active:</strong> Remaining trades are auto-adjusted by {compressionPercent}% to ensure handover deadline is met.
              </span>
            ) : (
        ...
        <div className="bg-slate-50/70 rounded-2xl border border-slate-200/60 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px]">
            ...
            <div className="p-3 bg-[#0066CC] text-white rounded-xl space-y-1.5 shadow-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
        <div className="p-3 rounded-xl border border-slate-150 bg-slate-50/50 flex items-start gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-sky-100 text-[#0066CC] flex items-center justify-center font-bold text-xs shrink-0">
            1
          </div>
          <div>
...
function ScheduleAnalysisReportModal({ projectContext, schedule, result, variance, lost, onClose }: ReportProps) {
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-[#0066CC]/90 backdrop-blur-md border border-white/20/40 backdrop-blur-sm flex justify-center overflow-y-auto p-4 md:p-10 no-print-overlay"
...
function AddStageDialog({ tasks, trades = [], onCancel, onApply }: {
  ...
  return (
    <div className="fixed inset-0 z-[210] bg-[#0066CC]/90 backdrop-blur-md border border-white/20/40 backdrop-blur-sm flex items-center justify-center p-5 no-print"
      onClick={e => { if (e.target === e.currentTarget) onCancel(); }}>
      <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
        ...
        <div className="flex gap-2 justify-end mt-6">
          ...
          <button onClick={handleApply} className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-[#0066CC]/90 backdrop-blur-md border border-white/20 hover:bg-[#0055B3] shadow-sm transition-all">
            Add Stage
          </button>
        </div>
      </div>
