...
const AIStatusIndicator: React.FC<AIStatusIndicatorProps> = ({ status }) => {
    ...
    return (
        <div className="relative group flex items-center justify-center">
            ...
            <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-max px-3 py-1.5 bg-slate-900/80 backdrop-blur-sm text-white text-xs font-bold rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none shadow-xl">
                {text}
                <div className="absolute bottom-[-4px] left-1/2 -translate-x-1/2 w-2 h-2 bg-slate-900/80 rotate-45"></div>
            </div>
        </div>
    );
};
