...
export default function HandoverDocketPage({ projectContext }: HandoverDocketPageProps) {
    ...
    const completedRooms = projectContext.rooms?.map(room => ({
        name: room.name,
        scope: room.scope?.join(', ') || 'Standard scope items'
    })) || [];

    const snags = projectContext.siteUpdates?.filter(u => u.type === 'snag') || [];
    const totalSnags = snags.length;
    const closedSnags = snags.filter(s => s.status === 'resolved' || s.status === 'acknowledged').length;
                                ...
                                <div className="p-6 border-2 border-[#C6A96C] rounded-lg relative overflow-hidden bg-[#faf8f5]">
                                    <div className="absolute -top-4 -right-4 w-24 h-24 bg-[#F3ECDD] rounded-full flex items-center justify-center border-4 border-white shadow-sm">
                                        <div className="text-center">
                                            ...
                                            <div className="text-lg font-black text-[#1E3A8A] leading-tight">6 MO</div>
                                        </div>
                                    </div>
                                    <div className="mb-4">
                                    ...
                                    <div className="text-[11px] text-slate-600 space-y-4 pr-16">
                                        ...
                                        <div>
                                            ...
                                            <p>Workmanship defects in executed carpentry, installation, finishing, and agreed trade works under normal usage for a period of 6 months from Handover Date.</p>
                                        </div>
...
fs.writeFileSync('components/client/HandoverDocketPage.tsx', content);
console.log('done');
