...
import { FullProjectData, ProjectContext, ProposalTier, Item, AIStrategy, MaterialSuggestion, TimelinePhase, LeadProfile, DecisionBrainOutput, ActiveProject, AIStatus, BoqItem, FullBoqItem, ExecutionBundle, SOFItem, ExecutionBundleStatus } from './types';
...
export default function App() {
    ...
    const [projectContext, setProjectContext] = useState<ProjectContext>(DEFAULT_CONTEXT);
    const [tiers, setTiers] = useState<ProposalTier[]>([]);
    const [activeTierId, setActiveTierId] = useState<string | null>(null);
    
    const [activeProject, setActiveProject] = useState<ActiveProject | null>(null); // Execution State
    ...
    const [leadProfile, setLeadProfile] = useState<LeadProfile>(DEFAULT_LEAD_PROFILE);
    const [decisionBrainOutput, setDecisionBrainOutput] = useState<DecisionBrainOutput | null>(null);

    // Client View Mode (for shared links/preview if implemented)
    const [isClientView, setIsClientView] = useState(false);
    const [clientViewData, setClientViewData] = useState<FullProjectData | null>(null);
    const [portalProjectId, setPortalProjectId] = useState<string | null>(null);
    const [appMode, setAppMode] = useState<'loading' | 'login' | 'ops' | 'client' | 'signoff' | 'selection_confirm' | 'agreement_signoff' | 'booking_approval' | 'mom_acknowledge'>('loading');
    const [signoffToken, setSignoffToken] = useState<string | null>(null);
    const [agreementSignoffToken, setAgreementSignoffToken] = useState<string | null>(null);
    const [selectionConfirmToken, setSelectionConfirmToken] = useState<string | null>(null);
    const [momToken, setMomToken] = useState<string | null>(null);
    const [clientPortalProject, setClientPortalProject] = useState<FullProjectData | null>(null);

    const [confirmReset, setConfirmReset] = useState(false);
    const [highlightedBankItemId, setHighlightedBankItemId] = useState<string | null>(null);
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

    // Ref to access active ID inside async effects without stale closures
    const activeIdRef = useRef(activeInternalId);
    useEffect(() => { activeIdRef.current = activeInternalId; }, [activeInternalId]);
    ...
    useEffect(() => {
        const init = async () => {
            ...
            const timeoutWrapper = <T,>(promise: Promise<T>, ms = 5000): Promise<T> => {
                return Promise.race([
                    promise,
            ...
            try {
                storedBank = await timeoutWrapper(db.getBank(), 5000);
            } catch (e) {
            ...
            if (newItems.length > 0 || bankModified) {
                console.log(`Merging ${newItems.length} new items into bank and fixing categories...`);
            ...
            try { storedDraftBank = await timeoutWrapper(db.getDraftBank(), 5000); } 
            catch(e) { 
            ...
            try { storedTemplates = await timeoutWrapper(db.getTemplates(), 5000) || INITIAL_TEMPLATES; }
            catch(e) {
            ...
            try { storedProjects = await timeoutWrapper(db.getProjects(), 15000); }
            catch(e) {
                console.error("Failed to load projects from DB, falling back to local memory:", e);
            ...
            if (portalId) {
                ...
                if (savedMode === 'client' && localStorage.getItem('ffds_client_project_id') === portalId) {
        ...
        if (activeTab === 'projects') {
            db.getProjects().then(fetchedProjects => {
                setProjectLibrary(currentLib => {
                    ...
                    if (currentActiveId) {
                        const inMemoryActive = currentLib.find(p => p.id === currentActiveId);
                        if (inMemoryActive) {
                            const dbIndex = fetchedProjects.findIndex(p => p.id === currentActiveId);
                            ...
                            if (dbIndex === -1) {
                                ...
                                if (inMemoryActive.lastModified > fetchedProjects[dbIndex].lastModified) {
        ...
        if (projectContext?.adHocItems && projectContext.adHocItems.length > 0) {
            setBank(prevBank => {
                ...
                if (uniqueNewItems.length > 0) {
                    ...
                    db.saveBank(updated).catch(err => console.error("Failed to sync adhoc items to DB:", err));
                    return updated;
                }
                return prevBank;
            });
        ...
        return tiers.map(tier => {
            ...
            tier.boq.forEach(b => {
                ...
                if (item) {
                    ...
                    itemCost = cost;
                    itemSell = calculateSellPrice(item.materials, item.labor, margin) * b.qty;
                } else {
            ...
            // Design Fee Calc
            let designFee = 0;
            if (projectContext.designFeeType === 'fixed_lumpsum') designFee = projectContext.designFee || 0;
            else if (projectContext.designFeeType === 'fixed_sqft') designFee = (projectContext.designFee || 0) * (projectContext.area || 0);
    ...
    }, [projectContext, tiersWithCalculatedSummaries, activeTierId, activeProject, materialSuggestions, timelinePhases, leadProfile, decisionBrainOutput, activeInternalId]);

    // Auto-save Project to DB
    useEffect(() => {
        if (activeInternalId && projectContext) {
            console.log("Auto-save useEffect triggered for project:", activeInternalId);
    ...
    const setBoqForActiveTier: React.Dispatch<React.SetStateAction<BoqItem[]>> = (action) => {
        setTiers(prevTiers => prevTiers.map(tier => {
            if (tier.id === activeTierId) {
                const newBoq = typeof action === 'function' ? action(tier.boq) : action;
    ...
    const executionBoq = useMemo(() => {
        ...
        return (tier.boq || []).map(b => {
             ...
             const effectiveMargin = b.marginOverride ?? item.margin;
             return { ...item, ...b, id: b.id, margin: effectiveMargin } as FullBoqItem;
        });
    ...
    const handleCreateNewProject = () => {
        ...
        if (orgData?.tenantId) {
            initCommunicationLog(newId, orgData.tenantId).catch(err => console.error("Found error initializing comm track:", err));
        }
    };
    ...
    const handleQuickProjectUpdate = async (projectId: string, field: string, value: any) => {
    ...
    const handleAddItemsFromPrompt = (itemsToAdd: { item: Item; qty: number }[]) => {
        if (!activeTierId) return;
    ...
    const handleExportHtml = (fileName?: string) => {
        ...
        setTimeout(() => {
            ...
            if (!clientViewNode) {
                alert('Could not find proposal content. Ensure you are on the Client Proposal tab.');
                return;
            }
                    ...
                    cards.forEach(card => {
                        card.addEventListener('click', () => {
                            ...
                            cards.forEach(c => {
                                ...
                                c.classList.remove(
                                    'border-2', 'border-slate-800', 'bg-white', 'shadow-lg', 'scale-[1.02]', 'z-10', 'ring-2', 'ring-slate-100', // Active
                                ...
                                if (cId === selectedId) {
                                    // Set Active Styling
                                    c.classList.add('border-2', 'border-slate-800', 'bg-white', 'shadow-lg', 'scale-[1.02]', 'z-10', 'ring-2', 'ring-slate-100');
            ...
            const url = URL.createObjectURL(blob);
            const finalName = fileName ? fileName : `${(projectContext.name || 'Unnamed_Project').replace(/\s+/g, '_')}_Proposal`;
            const a = document.createElement('a');
            a.href = url;
    ...
    const handleStartExecution = () => {
        const tier = tiersWithCalculatedSummaries.find(t => t.id === projectContext.approvedTierId);
        if (!tier) {
        ...
        const createBundle = (id: string, name: string, trade: string, cats: string[]): ExecutionBundle | null => {
        ...
        const b1 = createBundle('b_civil', 'Civil Prep & Execution', 'Civil', ['Civil', 'Demolition']);
        const b2 = createBundle('b_mep', 'MEP Rough-in & Final', 'MEP', ['Electrical', 'Plumbing', 'MEP']);
        const b3 = createBundle('b_ceiling', 'False Ceiling & Framing', 'Gypsum', ['False Ceiling', 'Gypsum']);
        const b4 = createBundle('b_flooring', 'Flooring & Tiling', 'Civil', ['Flooring', 'Tiling']);
        const b5 = createBundle('b_carpentry', 'Custom Carpentry & Millwork', 'Carpentry', ['Carpentry', 'Modular', 'Kitchen', 'Wardrobe']);
        const b6 = createBundle('b_painting', 'Painting & Finishing', 'Painting', ['Painting', 'Finishing']);

        [b1, b2, b3, b4, b5, b6].forEach(b => { if (b) bundles.push(b); });
        ...
        executionBoq.filter(i => ['Flooring', 'Tiling', 'Carpentry', 'Modular', 'Kitchen', 'Wardrobe', 'Painting', 'Finishing', 'Electrical', 'Plumbing'].includes(i.cat)).forEach((item, index) => {
            const itemName = (item.name || '').toLowerCase();
            if (itemName.includes('provide') || itemName.includes('finish') || itemName.includes('fixture')) {
    ...
    // Render Logic
    const isProjectTab = !['projects', 'bank', 'templates', 'ai-settings', 'team', 'subscription', 'setup-wizard', 'studio-settings', 'saas-dashboard'].includes(activeTab);
    ...
    const showFloatingBar = hasProjectData && PROJECT_WORKFLOW_ROUTES.includes(activeTab);
            ...
            <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50">
                <div className="flex flex-col items-center gap-8">
                    ...
                    >
                        ...
                        <motion.circle 
                            cx="80" cy="80" r="76" 
                            stroke="url(#ring-gradient)" strokeWidth="1" strokeOpacity="0.3"
                            animate={{ scale: [1, 1.05, 1] }} 
                            transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                        />
                        <motion.circle 
                            cx="80" cy="80" r="62" 
                            stroke="url(#ring-gradient)" strokeWidth="2" strokeOpacity="0.5"
                        ...
                        <motion.circle 
                            cx="80" cy="80" r="46" 
                            stroke="url(#ring-gradient)" strokeWidth="4" strokeOpacity="0.9"
                            animate={{ scale: [1, 1.05, 1] }} 
                    ...
                    >
                        <div className="text-[11px] font-black tracking-[0.2em] text-slate-800 uppercase">
    ...
    if (appMode === 'client' && clientPortalProject) {
        return <ClientPortal 
            ...
            onProjectUpdate={async (updatedProject) => {
                ...
                setProjectLibrary(prev => prev.map(p => p.id === updatedProject.id ? updatedProject : p));
            }}
        />;
    }

    if (appMode === 'ops' && currentRole === 'Site Supervisor') {
        const myTeamMember = teamMembers?.find(m => m.email.toLowerCase() === currentUserAuth?.email?.toLowerCase());

        return (
            <SupervisorMobileApp 
                projects={projectLibrary.filter(p => {
                    if (!p.context || !['won', 'execution', 'work_paused', 'completed'].includes(p.context.status || '')) return false;
    ...
    if (isClientView && clientViewData) {
        return (
            <div className="p-4 md:p-8">
              <ClientTab 
                tiers={clientViewData.tiers.map(t => ({...t, projectContext: clientViewData.context as ProjectContext}))}
                bank={[]}
    ...
    return (
        <div className="min-h-screen overflow-x-hidden relative">
            ...
            <div className={`fixed inset-0 bg-slate-900/50 z-[70] md:hidden transition-opacity ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsMobileMenuOpen(false)} />
            ...
            <main className="md:ml-64 w-full md:w-[calc(100%-16rem)] relative min-h-screen flex flex-col bg-slate-50">
                {isProjectTab && hasProjectData && activeTab !== 'studio-settings' && (
                ...
                <div className={`flex-grow ${activeTab === 'client' || activeTab === 'client-boq-pack' ? '' : 'p-8'}`}>
                    <AnimatePresence mode="wait">
                        ...
                        >
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-end gap-3 items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm max-w-7xl mx-auto">
                                        <div className="text-sm font-medium text-slate-700">
                                            Currently Editing: <span className={isDraftBankMode ? 'text-amber-600 font-bold' : 'text-emerald-600 font-bold'}>{isDraftBankMode ? 'Draft Sandbox' : 'Live Item Bank'}</span>
                                        </div>
                                        <button 
                                        ...
                                        {isDraftBankMode && (
                                            <button 
                                                onClick={() => {
                                                    if(confirm('Are you sure you want to completely overwrite the LIVE item bank with your Draft sandbox? This will affect new projects and prices.')) {
                                                        setBank(draftBank);
                                        ...
                                        {!isDraftBankMode && (
                                            <button 
                                                onClick={() => {
                                                    if(confirm('This will wipe your current Sandbox Draft and mirror the Live Bank. Continue?')) {
                                                        setDraftBank(bank);
                                                        setIsDraftBankMode(true);
                                                    }
                                                }}
                                                className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-bold hover:bg-slate-800 shadow-sm transition-colors"
                                            >
                            ...
                            {activeTab === 'ai-settings' && (
                                <AIStrategyTab aiStrategy={aiStrategy} setAiStrategy={setAiStrategy} />
                            )}
                            ...
                            {['studio-settings', 'team', 'subscription', 'setup-wizard'].includes(activeTab) && (
                                <StudioSettingsShell 
                                    activeTab={activeTab}
                                    setActiveTab={setActiveTab}
                            ...
                            {isProjectTab && hasProjectData ? (
                                <>
                                    ...
                                    {activeTab === 'onboarding' && (
                                        <OnboardingTab projectContext={projectContext} setProjectContext={setProjectContext} activeTier={activeCalculatedTier} />
                                    )}
                                    {activeTab === 'emails' && (
                                        <EmailDraftsTab projectContext={projectContext} tiers={tiersWithCalculatedSummaries} />
                                    )}
                                    {activeTab === 'comms-tracker' && (
                                        <CommunicationTracker 
                                            ...
                                            projectContext={projectContext}
                                            teamMembers={[]} 
                                            currentUserName={currentUserAuth?.displayName || currentUserAuth?.email || 'Unknown User'} 
                                    ...
                                    { (activeTab === 'site-ops' || activeTab === 'update-client-feed' || activeTab === 'record-decision') && (
                                        <SiteOpsTab 
                                            key={activeTab}
                                            projectContext={projectContext} 
                                            setProjectContext={setProjectContext} 
                                            ...
                                            initialModule={activeTab === 'update-client-feed' ? 'client-updates' : activeTab === 'record-decision' ? 'decision-tracker' : 'execution'}
                                            onAddCalculatedItem={(name, cat, qty, unit, roomId) => {
                                                if (!activeTierId) return;
                                ...
                                isProjectTab && !hasProjectData && (
                                    <div className="flex flex-col items-center justify-center h-[50vh]">
                                        <p className="text-slate-400 mb-4">No active project selected.</p>
                                        <button onClick={() => setActiveTab('projects')} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-bold">Go to Projects</button>
                                    </div>
                                )
                            )}
                        </MotionDiv>
